...
def fts_query(chat_db: Path, term: str, limit: int):
    ...
    q = """
      SELECT message_id, conversation_id, author_role, created_at,
             snippet(messages_fts, 4, '[', ']', '…', 18) as snip
      FROM messages_fts
...
def semantic_query(sem_db: Path, query: str, limit: int, min_len: int = 120):
...
def main():
    ...
    ap.add_argument('--chat-db', default=str(Path(__file__).resolve().parent / 'chatgpt.sqlite'))
    ap.add_argument('--sem-db', default=str(Path(__file__).resolve().parent / 'semantic.sqlite'))
    ap.add_argument('--fts-limit', type=int, default=40)
    ap.add_argument('--sem-limit', type=int, default=12)
    args = ap.parse_args()

    slug = sanitize_slug(args.slug)
    outdir = Path(__file__).resolve().parent / '_canon' / slug
    ...
    with log_event(
        ...
        message=f"Extract universe bundle: {slug}",
        tags=['canon'],
    ) as ev:
        ...
        # 3) Seed canon draft
        canon = [f"# Canon draft — {slug}", f"Generated: {generated}", "", "## Anchors fournis"]
        for t in args.terms:
            canon.append(f"- {t}")
        canon += [
            "",
