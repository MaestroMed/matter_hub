...
Shipped 2026-05-20: New `VoiceCloneKit` module under `mind/Modules/VoiceCloneKit/Sources/` ships four pure types — `ElevenLabsTokenStore` (Keychain wrapper, service `app.mind.ios.elevenlabs`, account `api-key`, same shape as NotionTokenStore), `ElevenLabsClient` (actor wrapping `POST /v1/voices/add`, `POST /v1/text-to-speech/<voice_id>`, `GET /v1/voices`, `GET /v1/user`; multipart body + synthesis payload exposed as pure statics for tests; default model `eleven_multilingual_v2` for FR pitch fidelity), `VoiceSampleRecorder` (`@MainActor @Observable` AVAudioRecorder wrapper, 44.1 kHz mono PCM 16-bit WAV, 24 Hz peak-level meter sampling for the 30-bar visualizer, samples written to `Documents/voice-samples/<timestamp>.wav`), and `AuditPitchAudioStore` (on-disk MP3 cache keyed by audit UUID under `Documents/audit-audio/`). New `VoiceCloneSetupSheet` in `mind/App/Sources/` walks Mehdi through 5 steps (paste API key + Keychain save → list existing voices auto-detecting "Mehdi Nafaa" → record 60-180 s WAV with live peak visualizer → upload to ElevenLabs → test playback). New `pitchAudioCard` section in `AuditSheet.swift` mounts between the pitch text + the save bar: when a voice is cloned (gated on `MINDPreferences.currentElevenLabsVoiceID()`) it surfaces a "Générer le pitch audio" CTA → synthesizes via ElevenLabs → caches MP3 → exposes inline AVAudioPlayer play/pause + "Régénérer" CTA. Empty state hints back to Settings. `AuditReport.audioPitchMP3Path` extends the value type as an optional backward-compatible Codable field. `HTMLTemplates.pitchAudioSection` inlines the cached MP3 as a base64 data URL inside `<audio controls preload="metadata">` so the Client Portal stays single-folder self-contained. `MINDPreferences` adds `elevenLabsVoiceID` + `elevenLabsVoiceName` (App Group UserDefaults). `Settings.SettingsView` adds a "Voix clonée" section between Linear + Intégrations dev with a "Configurer la voix" / "Gérer la voix" CTA forwarded to the host via the new `onPresentVoiceClone` closure (same surface as `onPresentBulkImport`). `Localizable.xcstrings` ships 35 FR/EN keys (`settings.voiceClone.*`, `voiceClone.*`, `audit.pitchAudio.*`). `Project.swift` updates `NSMicrophoneUsageDescription` to reflect the voice-clone use case. 19 new tests across `ElevenLabsClientTests` (10 — multipart body byte shape, synthesis payload JSON, voices envelope decoding, malformed-JSON error path), `ElevenLabsTokenStoreTests` (5 with XCTSkipIf simulator), `VoiceCloneFlowTests` (4 — state enum cases + AuditPitchAudioStore async round-trip). Suite at 1072 tests, 24 skipped, 0 failures (was 1053 in v1.0-alpha.17). iOS Simulator build SUCCEEDED. Vision verify at `mind/screenshots/v1.0-alpha.18.png` — host launches clean with the lead inbox + KPI bar populated (4 leads, 5 actifs, 830 €/mo).
...
**Watch target install caveat**: the watchOS simulator runtime isn't installed on Mehdi's Mac M5 Pro yet (only the watchOS SDK is — `xcrun simctl list runtimes` returns iOS 26.5 only). The `MINDWatch` target is defined and compiles in isolation, but its embed-in-App dependency (`.target("MINDWatch", condition: .when([.ios]))`) is commented out in `Project.swift` so the iOS test scheme builds without requiring the watchOS runtime. To pair it: (1) Xcode → Settings → Platforms → install watchOS 26.5, (2) uncomment the dependency line in `Project.swift`, (3) `tuist generate`. The bridge value-type contract + the screenshot proof on the iPhone side both ship today.
