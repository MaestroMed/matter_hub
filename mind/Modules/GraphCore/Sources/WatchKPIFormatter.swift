...
/// v1.0-alpha.17 — Pure formatters shared between the iPhone host
/// and the Apple Watch UI. The Watch target ships a thin duplicate
/// (Watch/Sources/WatchKPIFormatter.swift) with the same shape so the
/// tests on the iPhone side lock both bytewise. Every helper is a
/// pure function on `Int` / `String` — no Locale, no Date — so the
/// visual output is identical on the wrist and on the larger device
/// regardless of region settings.
public enum WatchKPIFormatter {

    /// Compact monthly recurring revenue glyph the Watch displays
    /// next to the EUR pill. Mirrors `WidgetMRRFormatter.compact`
    /// (the iOS 26 widget formatter) so the Lock Screen and the
    /// wrist glance render the same shape.
    ///
    /// - <1000: "830€"
    ...
    /// Maximum transcript preview length displayed on the lead row.
