...
public enum Module: String, CaseIterable {
    ...
    public func target() -> Target {
        .target(
            name: rawValue,
            // v1.0-alpha.15 — Modules ship to iPhone, iPad, and Mac
            // Catalyst. The historical `FocusKit → ActivityKit` reach
            // is now guarded behind `#if !targetEnvironment(macCatalyst)`
            // so FocusKit cross-compiles cleanly with the Live Activity
            // surface no-oped on Catalyst (see FocusController.swift).
